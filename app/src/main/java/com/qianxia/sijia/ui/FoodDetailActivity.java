...
import com.qianxia.sijia.adapter.FoodCommentAdapter;
...
public class FoodDetailActivity extends BaseActivity implements OnReplayListener<FoodComment> {
    ...
    private Food food;
    private ArrayList<FoodComment> foodComments;
    private LinearLayout emoLayout;
    private ViewPager viewPager;

    private FoodComment mComment;
    ...
    private void initHeaderView() {
        List<BmobFile> pics = food.getPics();
        ...
        View headerContentView = getLayoutInflater().inflate(R.layout.header_price_fooddetail_refresh, listView, false);
...
//        listView.setOverScrollMode(View.OVER_SCROLL_NEVER);
        ...
        adapter = new FoodCommentAdapter(this, foodComments, this);
        adapter.setOnDeleteCommentListener(new OnDeleteCommentListener() {
            @Override
            public void onDeleteComment(final int position) {
                ...
                dialog.setOnDialogClickListener(new OnDialogClickListener() {
                    ...
                    public void onDialogPositiveClick(DialogFragment dialog) {
                        FoodComment comment = adapter.getItem(position);
                        CommentManager.deleteFoodComment(FoodDetailActivity.this, comment, new UpdateListener() {
                            @Override
...
//                log("onPullDownToRefresh.refresh");
                page = 0;
                CommentManager.getFoodCommentsByPage(FoodDetailActivity.this, food, 0, new OnFoodCommentsLoadedListener() {
                    @Override
                    public void onFoodCommentsLoaded(List<FoodComment> foodComments) {
                        adapter.addAll(foodComments, true);
            ...
            public void onPullUpToRefresh(final PullToRefreshBase<ListView> refreshView) {
                page += 1;
                CommentManager.getFoodCommentsByPage(FoodDetailActivity.this, food, page, new OnFoodCommentsLoadedListener() {
                    @Override
                    public void onFoodCommentsLoaded(List<FoodComment> foodComments) {
                        adapter.addAll(foodComments, false);
    ...
    private void initHeaderContentView(View headerContentView) {
        TextView tvFoodName = (TextView) headerContentView.findViewById(R.id.tv_foodname_fooddetail_header2);
        TextView tvShopName = (TextView) headerContentView.findViewById(R.id.tv_shopname_fooddetail_header2);
        TextView tvAddress = (TextView) headerContentView.findViewById(R.id.tv_address_fooddetail_header2);
        TextView tvOpenTime = (TextView) headerContentView.findViewById(R.id.tv_opentime_fooddetail_header2);
        TextView tvPrice = (TextView) headerContentView.findViewById(R.id.tv_price_fooddetail_header2);
        TextView tvAuthor = (TextView) headerContentView.findViewById(R.id.tv_anthor_fooddetail_header2);
//        RatingBar ratingBar = (RatingBar) headerContentView.findViewById(R.id.ratingbar_fooddetail_header2);
        TextView tvDescription = (TextView) headerContentView.findViewById(R.id.tv_description_fooddetail_header2);
        tvCommentNum = (SijiaTextView) headerContentView.findViewById(R.id.tv_commentnum_fooddetail_header2);
        tvLikerNum = (SijiaTextView) headerContentView.findViewById(R.id.tv_likenum_fooddetail_header2);
        tvFoodName.setText(food.getName());
        tvShopName.setText("佳店：" + food.getShop().getName());
        tvAddress.setText(food.getShop().getAddress());
        String openTime = food.getShop().getOpenTime();
        log("tvShopName:" + food.getShop().toString());

        SijiaUser user = food.getAuthor();
        String title = "";
        if (user != null) {
            title = "该佳食信息由 " + user.getUsername() + " 提供";
        } else {
            title = "该佳食信息由 " + "Test" + " 提供";
        }
...
//                    return;
//                }
//                Intent intent =  new Intent(FoodDetailActivity.this,PostCommentActivity.class);
...
//        });
        tvAddress.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(FoodDetailActivity.this, MapActivity.class);
                intent.putExtra("from", "food");
                intent.putExtra("shop", food.getShop());
                jumpTo(intent, false);
            }
...
//                    log("hasZan:"+String.valueOf(hasZan));
                ...
                isUpdateZan = true;
                if (hasZan) {
                    mLike.delete(FoodDetailActivity.this, new DeleteListener() {
                        @Override
                        public void onSuccess() {
//                            log("deletelike");
                            int likerNum = food.getLikerNum();
                            likerNum -= 1;
                            food.setLikerNum(likerNum);
                            final int finalLikerNum = likerNum;
                            food.update(FoodDetailActivity.this, new UpdateListener() {
                                @Override
                                public void onSuccess() {
                    ...
                    like.setFoodId(food.getObjectId());
                    like.setLikerId(currentUser.getObjectId());
                    like.save(FoodDetailActivity.this, new SaveListener() {
                        @Override
                        public void onSuccess() {
                            BmobQuery<Like> query = new BmobQuery<>();
                            query.addWhereEqualTo("foodId", food.getObjectId());
                            query.addWhereEqualTo("likerId", currentUser.getObjectId());
                            query.findObjects(FoodDetailActivity.this, new FindListener<Like>() {
                                @Override
...
//            log(like.toString());
            ...
            query.addWhereEqualTo("foodId", like.getFoodId());
    ...
    public void sendReplay(View view) {
        ...
        FoodReply replay = new FoodReply();
        ...
        replay.setFoodComment(mComment);
...
//                toast("回复发布成功");
    ...
    @OnClick(R.id.fl_fooddetail_root)
    ...
    private void refresh() {
        ...
        query.addWhereEqualTo("objectId", food.getObjectId());
        query.include("shop.author");
        ...
        CommentManager.getFoodCommentsByPage(this, food, 0, new OnFoodCommentsLoadedListener() {
            @Override
            public void onFoodCommentsLoaded(List<FoodComment> foodComments) {
                adapter.addAll(foodComments, true);
//                refreshListView.onRefreshComplete();
    ...
    @Override
    public void onReplay(FoodComment comment, String title) {
    ...
    private void share() {
        log(food.getShop().getName() + ":" + food.getShop().getAddress());
...
// text是分享文本，所有平台都需要这个字段
        ...
        oks.setText(food.getName() + "\n" + food.getDescription());
// imagePath是图片的本地路径，Linked-In以外的平台都支持此参数
//oks.setImagePath("/sdcard/test.jpg");//确保SDcard下面存在此张图片
        oks.setImageUrl(food.getPics().get(0).getUrl());
        oks.setTitleUrl(food.getPics().get(0).getUrl());
...
// 启动分享GUI
    ...
    private void collect() {
        ...
        collect.setContent(food.getDescription());
        collect.setFoodId(food.getObjectId());
        collect.setImgUrl(food.getPics().get(0).getUrl());
        collect.setTime(System.currentTimeMillis());
        collect.setName(food.getName());
