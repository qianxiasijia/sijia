 ...
 */
public class FoodCommentAdapter extends SijiaBaseListAdapter<FoodComment> {
    private final LruCache<String, Bitmap> memoryCache;
    private OnReplayListener<FoodComment> listener;
    ...
    public FoodCommentAdapter(Context context, List datas, OnReplayListener<FoodComment> listener) {
        super(context, datas);
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        final FoodComment comment = getItem(position);
...
//            Log.i("DEBUG",imgUrls.length()+"");
            ...
            viewHolder.tvDelete.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    if (deleteListener != null) {
                        deleteListener.onDeleteComment(position);
                    }
                }
            });
    ...
    private void showReplays(FoodComment comment, final LinearLayout replayContainer) {

        ReplayManager.getFoodReplays(mContext, comment, new OnFoodReplayLoadedListener() {
            @Override
            public void onFoodReplayLoaded(List<FoodReply> list) {
                if (list.size() > 0) {
